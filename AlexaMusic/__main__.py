...
async def init():
    if all(not getattr(config, f"STRING{i}") for i in range(1, 6)):
        LOGGER("AlexaMusic").error("Pyrogram string session eksik. Lütfen yapılandırın.")
        exit()
    ...
    LOGGER("AlexaMusic.plugins").info("Tüm eklentiler başarıyla yüklendi.")
    ...
    try:
        ...
        LOGGER("AlexaMusic").error("Sesli sohbet aktif değil. Lütfen açın.")
        exit()
    except Exception:
        pass
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("AlexaMusic").info("Alexa Music Bot durduruluyor...")
